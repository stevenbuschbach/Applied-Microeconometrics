/***********************
Table F5: SHSUE DD Estimates for Utilization (unweighted, 50+)
***********************/
...
log using "$output/log_tableF5", replace text	
...
local r replace
	foreach var of varlist YBP_rsrce YCP_tot_vis YBP_dphys YBP_pdrug_oop{
		*POOLED MODEL TO GET p-value on TEST of DD coefs across income groups
		...
		areg `var' `X' POOL*, cluster(PFpsu) a(PFpsu)	
		test POOL300D70xNHC
		local p300 = r(p)
		test POOL100D70xNHC
		local p100 = r(p)
		...
		*RUN THE SEPARATE MODELS TO GET COEFS
		foreach pov of numlist 0 100 300{
			...
			xi, prefix(N): areg `var' `X' if PF==`pov' , cluster(psu) a(psu)
			sum `var' if D70!=1 & NHC==1  & PF==`pov'
			outreg2 D70xNHC using "$output/tableF5.xls", `r' noparen noaster ctitle("POV`pov': `varlab'") addstat(MDV, r(mean), "A=B (p-val)", `p100', "A=C (p-val)", `p300')
			local r append
		}
	}
