/*********************************************************************
FIGURE H1: Predicting the timing of CHC grant with 1970 Characteristics, Later Centers
*********************************************************************/
clear
clear matrix
clear mata
...
pause on
capture log close
log using "$output/log_figureH1", replace text	

use *fips* year chc_year_exp amr _* copop using "$data/aer_data" if chc_year_exp>1974 & chc_year_exp<=., clear
drop if fips==6037|fips==17031|fips==36061

*add the 1965 AMR value to all obs and then keep year 1960
egen amr75 = total(amr*(year==1975)), by(fips)
keep if year==1970
gen damr = amr75-amr
...
reg amr75 chc_year_exp [aw = copop], robust
predict amrhat
...
save "$output/amrhatadj", replace
restore
merge 1:1 stfips cofips  using "$output/amrhatadj"
drop _merge
...
save "$output/damrhatadj", replace
restore
merge 1:1 stfips cofips  using "$output/damrhatadj"
drop _merge 
...
xlabel(1975(1)1980, labsize(large))				/*XLABEL: this makes the axis tight [it doesn't work with xscale(range()), only xlabel]*/
ylabel(, labsize(large) axis(1))						/*YLABEL: make there be 4 ticks so they don't overlap and make the tick-mark text small*/
...
title("{it: A. 1975 AMR}", size(vlarge) color(black))	/*TITLE: panel titles are defined in the locals above*/
legend(off)
graphregion(fcolor(white) color(white) icolor(white)) 
...
title("{it: B. 1970-1975 Change in AMR}", size(vlarge) color(black))	/*TITLE: panel titles are defined in the locals above*/
legend(off)
graphregion(fcolor(white) color(white) icolor(white)) 
yaxis(1)
||
...
graph export "$output/figureH1.wmf", replace
