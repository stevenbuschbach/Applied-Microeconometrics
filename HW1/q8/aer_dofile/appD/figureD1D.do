/*********************************************************************
FIGURE D1D: Predicting the timing of CHC grant with 1960 Characteristics
*********************************************************************/
clear
clear matrix
clear mata
...
pause on
capture log close
log using "$output/log_figureD1D", replace text	

use *fips* year chc_year_exp amr_eld _* copop_eld using "$data/aer_data" if chc_year_exp<=1974, clear
...
ylabel(2000(1000)5000, labsize(large) axis(1))						/*YLABEL: make there be 4 ticks so they don't overlap and make the tick-mark text small*/
...
legend(off)										/*LEGEND: off because grc1leg only needs one legend to work*/
xlabel(1965(3)1974, labsize(large))				/*XLABEL: this makes the axis tight [it doesn't work with xscale(range()), only xlabel]*/
ylabel(-1000(500)1000, labsize(large) axis(1))						/*YLABEL: make there be 4 ticks so they don't overlap and make the tick-mark text small*/
xtitle("")										/*XTITLE: no title on this one, only on the bottom panels to save vertical space*/
...
graph export "$output/figureD1D.wmf", replace
