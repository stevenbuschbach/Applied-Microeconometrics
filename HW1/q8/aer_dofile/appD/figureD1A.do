/*********************************************************************
FIGURE D1A: Predicting the timing of CHC grant with 1960 Characteristics
*********************************************************************/
clear
clear matrix
clear mata
...
pause on
capture log close
log using "$output/log_figureD1A", replace text	

use *fips* year chc_year_exp imr _* births using "$data/aer_data" if chc_year_exp<=1974, clear
...
ylabel(0(20)60, labsize(large) axis(1))						/*YLABEL: make there be 4 ticks so they don't overlap and make the tick-mark text small*/
...
title("{it: A. 1965 IMR}", size(vlarge) color(black))	/*TITLE: panel titles are defined in the locals above*/
legend(off)
graphregion(fcolor(white) color(white) icolor(white)) 
...
legend(off)										/*LEGEND: off because grc1leg only needs one legend to work*/
xlabel(1965(3)1974, labsize(large))				/*XLABEL: this makes the axis tight [it doesn't work with xscale(range()), only xlabel]*/
ylabel(-40(20)20, labsize(large) axis(1))						/*YLABEL: make there be 4 ticks so they don't overlap and make the tick-mark text small*/
xtitle("")										/*XTITLE: no title on this one, only on the bottom panels to save vertical space*/
ytitle("Change in Deaths per 100,000 Residents", size(large))
title("{it: B. 1960-1965 Change in IMR}", size(vlarge) color(black))	/*TITLE: panel titles are defined in the locals above*/
legend(off)
graphregion(fcolor(white) color(white) icolor(white)) 
yaxis(1)
||
...
graph export "$output/figureD1A.wmf", replace
