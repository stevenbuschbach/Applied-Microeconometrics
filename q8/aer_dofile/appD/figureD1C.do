/*********************************************************************
FIGURE D1C: Predicting the timing of CHC grant with 1960 Characteristics
*********************************************************************/
clear
clear matrix
clear mata
...
pause on
capture log close
log using "$output/log_figureD1C", replace text	

use *fips* year chc_year_exp amr_ad _* copop_ad using "$data/aer_data" if chc_year_exp<=1974, clear
...
ylabel(0(200)600, labsize(large) axis(1))						/*YLABEL: make there be 4 ticks so they don't overlap and make the tick-mark text small*/
...
graph export "$output/figureD1C.wmf", replace
