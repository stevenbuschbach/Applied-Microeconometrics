/*********************************************************************
FIGURE D1B: Predicting the timing of CHC grant with 1960 Characteristics
*********************************************************************/
clear
clear matrix
clear mata
...
pause on
capture log close
log using "$output/log_figureD1B", replace text	

use *fips* year chc_year_exp amr_ch _* copop_ch using "$data/aer_data" if chc_year_exp<=1974, clear
...
legend(off)										/*LEGEND: off because grc1leg only needs one legend to work*/
xlabel(1965(3)1974, labsize(large))				/*XLABEL: this makes the axis tight [it doesn't work with xscale(range()), only xlabel]*/
ylabel(0(100)300, labsize(large) axis(1))						/*YLABEL: make there be 4 ticks so they don't overlap and make the tick-mark text small*/
xtitle("")										/*XTITLE: no title on this one, only on the bottom panels to save vertical space*/
...
lcolor(forest_green maroon)									/*LINE COLOR:*/ 
xlabel(1965(3)1974, labsize(large))				/*XLABEL: this makes the axis tight [it doesn't work with xscale(range()), only xlabel]*/
ylabel(-100(100)200, labsize(large) axis(1))						/*YLABEL: make there be 4 ticks so they don't overlap and make the tick-mark text small*/
xtitle("")										/*XTITLE: no title on this one, only on the bottom panels to save vertical space*/
title("{it: B. 1960-1965 Change in AMR}", size(vlarge) color(black))	/*TITLE: panel titles are defined in the locals above*/
...
graph export "$output/figureD1B.wmf", replace
