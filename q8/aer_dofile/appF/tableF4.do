/***********************
Table F4: SHSUE DD Estimates for Utilization (all ages)
***********************/
...
log using "$output/log_tableF4", replace text	
	...
	foreach var of varlist YBP_rsrce YCP_tot_vis YBP_dphys YBP_pdrug_oop{
		...
		*RUN THE SEPARATE MODELS TO GET COEFS
		foreach pov of numlist 0 100 300{
			...
			outreg2 D70xNHC using "$output/tableF4.xls", `r' noparen noaster ctitle("POV`pov': `varlab'") addstat(MDV, r(mean), "A=B (p-val)", `p100', "A=C (p-val)", `p300')
			local r append
		}
	}
