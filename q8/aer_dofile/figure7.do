/******************************************************
FIGURE 7: Age-Group-Specific Event-Study Results for ASMR
******************************************************/
clear
clear matrix
set mat 1000
...
pause on
capture log close
log using "$output/log_figure7", replace text	

*save a file to hold the coefficients for stata graphs
set obs 23
gen time = _n - 8
save "$output/asmr_chc_es_results", replace emptyok
...
preserve
	use "$output/asmr_chc_es_results", clear
	quietly{
		gen b_X_inf				= .
		gen se_X_inf				= .	
		forval h = 1/23{
	...
	save "$output/asmr_chc_es_results", replace
restore
...
*older adults panel
	...
	graph export "$output/figure7.wmf", replace
