...
pause on
capture log close
log using "$output/log_figureE2", replace text	

*save a file to hold the coefficients for stata graphs
set obs 23
gen time = _n - 8
save "$output/amr_chc_es_urban_results_uw", replace emptyok
...
preserve
	use "$output/amr_chc_es_urban_results_uw", clear
	quietly{
		gen b_X_high				= .
		gen se_X_high				= .	
		gen b_X_low					= .
	...
	save "$output/amr_chc_es_urban_results_uw", replace
restore
...
graph export "$output/figureE2.wmf", replace

erase "$output/amr_chc_es_urban_results_uw.dta"
log close
exit
