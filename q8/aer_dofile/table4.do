/******************************************************
TABLE 4: Heterogeneity in older adult DD Estimates
******************************************************/
clear
clear matrix
set mat 5000
...
pause on
capture log close
/*log using "$output/log_table4", replace text	

use "$data/aer_data" if year<=1988, clear

*define above/below median 
gen treat1 = chc_year_exp<=1974
sum amr_eld if year==1960 & treat1, det
local m = r(p50)
egen hamr = total(treat1*(year==1960)*(amr_eld>`m')), by(fips)

gen pcmd = _tot_act_md/copop
sum pcmd if year==1960 & treat1, det
local m = r(p50)
egen hpcmd = total(treat1*(year==1960)*(pcmd>`m')), by(fips)
...
foreach var of varlist hamr hpcmd{
		...
		*pre-treatment AMR means by group
		sum amr_eld if `var' & chc_year_exp<=1974 & exp1==-1 [aw=copop_eld]
		...
		*regression
		...
		test Low_DDdid1_2 == High_DDdid1_2
		test Low_DDdid1_4 == High_DDdid1_4, accumulate
		test Low_DDdid1_5 == High_DDdid1_5, accumulate
		test Low_DDdid1_6 == High_DDdid1_6, accumulate
		local hp = r(p)
		outreg2 using "$output/table4.xls", `r' keep(Low_DDdid1_2 Low_DDdid1_4 Low_DDdid1_5 Low_DDdid1_6 High_DDdid1_2 High_DDdid1_4 High_DDdid1_5 High_DDdid1_6) noparen noaster ctitle("`var': `e(cmdline)'") addstat(lowmean, `prelow', highmean, `prehigh', htest, `hp')
		local r append
}
*/		
...
*stack by race
reshape long amr copop popwt dflpopwgt, i(stfips cofips year) j(nonwhite)
...
*pre-treatment AMR means by group
sum amr if chc_year_exp<=1974 & exp1==-1 & ~nonwhite [aw=copop]
local prew = r(mean)
sum amr if chc_year_exp<=1974 & exp1==-1 & nonwhite [aw=copop]
local prenw = r(mean)

xtreg amr `X' if d100r, cluster(fips) fe
test W_DDdid1_2 == NW_DDdid1_2
test W_DDdid1_4 == NW_DDdid1_4, accumulate
test W_DDdid1_5 == NW_DDdid1_5, accumulate
test W_DDdid1_6 == NW_DDdid1_6, accumulate			
local rp = r(p)

outreg2 using "$output/table4.xls", `r' keep(W_DDdid1_2 W_DDdid1_4 W_DDdid1_5 W_DDdid1_6 NW_DDdid1_2 NW_DDdid1_4 NW_DDdid1_5 NW_DDdid1_6) noparen noaster title(`e(depvar)') ctitle("`e(cmdline)'") addstat(whitemean, `prew', nonwhitemean, `prenw', rtest, `rp')
...
forval rg = 1/4{
	...
	sum amr_eld if chc_year_exp<=1974 & exp1==-1 & region~=`rg' [aw=copop_eld]
	local pre = r(mean) 
	
	xtreg amr_eld `X' if region~=`rg', cluster(fips) fe
	
	*OUTREG RESULTS
	outreg2 using "$output/table4.xls", `r' keep(_DDdid1_2 _DDdid1_4 _DDdid1_5 _DDdid1_6) noparen noaster ctitle("Dropping the `Reg`rg'': `e(cmdline)'") addstat(premean, `pre')
}
