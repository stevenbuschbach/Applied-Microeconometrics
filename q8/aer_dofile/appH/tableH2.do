/******************************************************
TABLE H2: Cause-specific DD Estimates for older adults (50+) and elderly/nonelderly, ALL CENTERS
******************************************************/
clear
clear matrix
set mat 5000
...
pause on
capture log close
log using "$output/log_tableH2", replace text	
...
*OUTREG RESULTS
outreg2 using "$output/tableH2.xls", `r' keep(_DDdid2_2 _DDdid2_4 _DDdid2_5 ) noparen noaster ctitle("AMR, 50+: `e(cmdline)'") 
local r append
forval c = 2/7{		
	xtreg amr_eld_`c' `X', cluster(fips) fe
			
	*OUTREG RESULTS
	outreg2 using "$output/tableH2.xls", `r' keep(_DDdid2_2 _DDdid2_4 _DDdid2_5 ) noparen noaster ctitle("AMR, 50+, Cause `c': `e(cmdline)'") 
}
...
replace asmr_5064 = asmr_5064*10
*all cause
xtreg asmr_5064 `X', cluster(fips) fe
		
*OUTREG RESULTS
outreg2 using "$output/tableH2.xls", `r' keep(_DDdid2_2 _DDdid2_4 _DDdid2_5 ) noparen noaster ctitle("AMR, 50-64: `e(cmdline)'") 

forval c = 2/7{		
	xtreg asmr_5064_`c' `X', cluster(fips) fe
			
	*OUTREG RESULTS
	outreg2 using "$output/tableH2.xls", `r' keep(_DDdid2_2 _DDdid2_4 _DDdid2_5 ) noparen noaster ctitle("AMR, 50-64, Cause `c': `e(cmdline)'") 
	local r append
}
...
replace asmr_6500 = asmr_6500*10
*all cause
xtreg asmr_6500 `X', cluster(fips) fe
		
*OUTREG RESULTS
outreg2 using "$output/tableH2.xls", `r' keep(_DDdid2_2 _DDdid2_4 _DDdid2_5 ) noparen noaster ctitle("AMR, 65+: `e(cmdline)'") 

forval c = 2/7{		
	xtreg asmr_6500_`c' `X', cluster(fips) fe
			
	*OUTREG RESULTS
	outreg2 using "$output/tableH2.xls", `r' keep(_DDdid2_2 _DDdid2_4 _DDdid2_5 ) noparen noaster ctitle("AMR, 65+, Cause `c': `e(cmdline)'") 
	local r append
}
