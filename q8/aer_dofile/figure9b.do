/******************************************************
FIGURE 9: ES Results for Hospital Capacity
******************************************************/
clear
clear matrix
clear mata
...
pause on
capture log close
log using "$output/log_figure9b", replace text	

*save a file to hold the coefficients for stata graphs
set obs 33
gen time = _n - 17
save "$output/aha_chc_es_results", replace emptyok

use "$data/aer_chc_aha.dta", clear
...
foreach y in bpc hpc{
	...
	*STORE RESULTS IN A STATA FILE
	use "$output/aha_chc_es_results", clear
	quietly{
		gen b_`y'							= .
		gen se_`y' 							= .	
		forval h = 1/33{
				if `h'==16{
					...
					replace b_`y' 			= _b[_Texp1_`h'] in `h'
					replace se_`y' 			= _se[_Texp1_`h'] in `h'
				}
		}
	}
	save "$output/aha_chc_es_results", replace
	restore
}


*trend breaks
local X "_Iyear* _IyeaXDu* _IyeaXst* D_* _Texp1_1 pre_exp1 dcons_post post_exp1 _Texp1_33 [aw=popwt]"	

cap drop treat
egen treat = total(exp1==0), by(fips)
*make the zero in the index come at -1, then multiply by a treatment dummy so that all other counties get zero
gen pre_exp1 = (exp1*(exp1>=-15)*(exp1<15) + 1)
replace pre_exp1 = 0 if treat~=1 | (exp1<-15 | exp1>=15)
*make the zero in the index come at -1, then multiply by a treatment dummy so that all other counties get zero		
gen post_exp1 = (exp1*(exp1>-1)+1)*(exp1<15)
replace post_exp1 = 0 if treat~=1 | (exp1<=-1 | exp1>=15)
gen dcons_post = exp1>-1 & exp1<15	

xtreg bpc `X', cluster(fips) fe	
global tb_bpc_b: display %4.3f _b[post_exp1]
global tb_bpc_se: display %4.3f _se[post_exp1]	

xtreg hpc `X', cluster(fips) fe	
global tb_hpc_b: display %5.4f _b[post_exp1]
global tb_hpc_se: display %5.4f _se[post_exp1]
...
*global tb_bpc_b: display %4.3f 0.008
*global tb_bpc_se: display %4.3f 0.017

*global tb_hpc_b: display %5.4f -0.0002
*global tb_hpc_se: display %5.4f 0.0001
...
/*Hospitals Per Capita*/		
	...
	twoway (scatter b_hpc ub*hpc* lb*hpc* time if time>=-15 & time<=15,	/*just the coefficients, not the confidence intervals...scatter allows markers, and is actually equivalent to "line" with the connect option*/
		...
		xlabel(-15(3)15, labsize(medium))  				/*XLABEL: this makes the axis tight [it doesn't work with xscale(range()), only xlabel]*/
		...
		title("{it: B. Number of Hospitals or Hospital beds per Capita}", size(medium) color(black))
		...
		title("{it: B. Number of Hospitals or Hospital beds per Capita}", size(medium) color(black))
		text(-.6 7.5  "Post-CHC Trend-Break Estimates:" "    Hospitals = $tb_hpc_b (s.e. =  $tb_hpc_se)" "    Beds =  $tb_bpc_b (s.e. =  $tb_bpc_se)", j(left) size(medsmall) yaxis(2))
		graphregion(fcolor(white) color(white) icolor(white) margin(small)) )
		
		(pcarrowi 1 -3 1 -1.5, lcolor(black) mcolor(black) lwidth(medthick) mlwidth(medthick) yaxis(2)
		text(1 -8 "Year Before CHC Began", j(left) size(medium) yaxis(2))
		saving("$output/panel_b", replace));
		#delimit cr;
