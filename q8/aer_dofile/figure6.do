/******************************************************
FIGURE 6: Event-Study Results for AMR by Urbanicity in 1960
******************************************************/
clear
clear matrix
set mat 1000
...
pause on
capture log close
log using "$output/log_figure6", replace text	

*save a file to hold the coefficients for stata graphs
set obs 23
gen time = _n - 8
save "$output/amr_chc_es_urban_results", replace emptyok
...
preserve
	use "$output/amr_chc_es_urban_results", clear
	quietly{
		gen b_X_high				= .
		gen se_X_high				= .	
		gen b_X_low					= .
		...
		forval h = 1/23{
				if `h'==7{
					...
					replace b_X_low = _b[_Texp1_`h'] in `h'
					replace se_X_low = _se[_Texp1_`h'] in `h'
					replace b_X_high = _b[HU_Texp1_`h'] in `h'
					replace se_X_high = _se[HU_Texp1_`h'] in `h'
				}
		}
	}
	save "$output/amr_chc_es_urban_results", replace
restore
...
twoway (scatter b_X_high b_X_low *bhi *blo
		...
		legend(off)
		xlabel(-6(3)15, labsize(medlarge))  			/*XLABEL: this makes the axis tight [it doesn't work with xscale(range()), only xlabel]*/
		...
		text(35 7 "Year Before CHCs Began Operating", j(left)))
		(pcarrowi 16 8 1 7, lcolor(black) mcolor(black) lwidth(medthick) mlwidth(medthick)
		text(22 10 "Effects of CHCs in counties with below" "median share urban population (average" "`lshare'% in non-urban areas)", j(left)))
		(pcarrowi -25 -0.5 -20.5 3.5, lcolor(black) mcolor(black) lwidth(medthick) mlwidth(medthick)
...
graph export "$output/figure6.wmf", replace
