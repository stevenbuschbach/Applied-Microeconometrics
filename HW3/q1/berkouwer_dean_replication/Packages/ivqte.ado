*! version 2.1.17  24jan2011  Markus Froelich and Blaise Melly
...
*2.1.16: check that qreg converged and report an error message if the convergence failed
*2.1.17: allow for the absence of regressor when using AAI, set a lower bound on the density when ivqte estimate the variance of FM to avoid missing standard errors
...
*if the option positive has been selected, then weighted quantile regression can be used to calculate the results
		...
		forvalues i=1/`nq'{
			...
			if r(convcode)==0{
				display as error "Convergence not achieved by the command qreg"
				error 430
			}
			matrix `temp1'=e(b)
